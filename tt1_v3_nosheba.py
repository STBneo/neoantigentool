...
RES = [ 'ALA','ARG','ASN','ASP','CYS','GLN','GLU','GLY','HIS','ILE','LEU','LYS','MET','PHE','PRO','SER','THR','TRP','TYR','VAL','HIE']
...
def acc_chem_count(sam,rfeats,folder,ref):
	...
	if not os.path.exists(sam + '_aa.out') :
		os.system(GEAR + '/enva_v1.4 -a ' + sam + '_rev.pdb > ' + sam + '_aa.out')
	...
	for pdb in pdbs:
		...
		if not os.path.exists(bbb[0] + 'B' + bbb[1] + '.pdb.env'):
...
#		if not os.path.exists(pdb.split('.')[0].replace('_h','') + '.ser'):
#			with open(pdb.split('.')[0].replace('_h','') + '.ser','w') as ff4:
#				for sid in sidx:
#					ff4.write('%s\n'%(sid))

#		cmd3 = GEAR + '/iskew ' + pdb.split('.')[0].replace('_h','') + '.ser >> total_sk.txt'
#		subprocess.call(cmd3,shell=True)

		if idx1 == 0 :
			with open(wdir1 + '/' + sam + '_energy_matrix/rmsd.txt','w') as rr:
				rr.write('PDB\trmsd\n')
		ext_chain(pdb.split('.')[0].replace('_h',''),'B','lig')
		cmdk = GEAR+ '/rmsd_total ' + ref + '_B ' + pdb.split('.')[0].replace('_h','') + '_B bb >> ' + wdir1 + '/' + sam + '_energy_matrix/rmsd.txt'
...
#	with open(wdir1 + '/' + sam + '_energy_matrix/total_sk1.txt','w') as ff5:
#		ff5.write('PDB\tskewness\tClass\tDecision\n')
#		with open('total_sk.txt','r') as ff6:
#			lines = ff6.readlines()
#			for line in lines:
#				if line.split('\t')[1] == 'nan':
...
def conv(aa,cc):
	...
	for pdb in pdbs:
		...
		if cc == 'sim_conf':
			...
			ser = pdb.split('_')[5].split('.')[0] + '_1ST'
		elif cc == 'PDB_2ND':
			ser = pdb.split('_')[5].split('.')[0] + '_2ND'
...
if len(sys.argv) == 1:
	...
	try:
		if not os.path.exists(sam + '/' + sam.replace('_A0201','') + '_energy_matrix'):
			os.mkdir(sam + '/' + sam.replace('_A0201','') + '_energy_matrix' ,0777)
	except OSError:
		pass
	...
	with open(sam1 + '_cl.pdb','w') as ffx:
		with open(sam1 + '.pdb','r') as ff:
			lines = ff.readlines()
			for line in lines:
	...
	ext_chain(sam1 + '_cl','A','rec')
	ext_chain(sam1 + '_cl','B','lig')
#	with open(sam + '_cl_A.pdb','w') as ff5:
#		with open(sam + '_cl.pdb','r') as ffx:
#			lines = ffx.readlines()
#			for line in lines:
...
#	with open(sam + '_cl_B.pdb','w') as ff6:
#		with open(sam + '_cl.pdb','r') as ffx:
#			lines = ffx.readlines()
#			for line in lines:
...
#		ff6.write('END\n')
	...
	if not os.path.exists(rec + '.out') :
		cmd = GEAR + '/enva_v1.4 -m ' + rec + '.pdb xxxxB > ' + rec + '.out'
		subprocess.call(cmd,shell=True)

	rfeats = []
	with open(rec + '.out','r') as f1:
		lines = f1.readlines()
		for line in lines:
...
#	df_sk = pd.read_csv(sam + '/' + sam + '_energy_matrix/total_sk1.txt',sep='\t')
	...
	df_final1.to_csv(sam + '/' + sam.replace('_A0201','') + '_energy_matrix/' + sam + '_full_env.txt',sep='\t',index=False,na_rep='-')
